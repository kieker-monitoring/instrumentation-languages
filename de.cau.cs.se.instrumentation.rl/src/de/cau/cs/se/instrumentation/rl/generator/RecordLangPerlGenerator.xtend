...
class RecordLangPerlGenerator extends RecordLangGenericGenerator {
	...
	override createContent(RecordType type, String author, String version) '''
		...
		=head1 SYNOPSIS
		 ...
		 my $record = «type.recordName»->new(«type.compileProperties.createParameterCall»);
		...
		=head2 $record = «type.recordName»->new(«type.compileProperties.createParameterCall»);
		...
		sub new {
		  my («type.compileProperties.createParameterCall») = @_;
		  my $this = {
		    «type.compileProperties.map[createProperty].join(',\n')»
		  };
	 ...
	 */
	override directoryName(RecordType type) '''perl«File::separator»«(type.eContainer as Model).name.replace('.',File::separator)»'''
		
	/**
	 * Return the extension used for the file type generated by this generator. 
	 */
	override getExtension() '''pm'''
