...
#include <stdlib.h>
#include <kieker.h>
#include "c/kieker/common/record/controlflow/kieker_common_record_controlflow_operation_execution_record.h"
...
int kieker_common_record_controlflow_operation_execution_record_serialize(char *buffer, const int id, const int offset, const kieker_common_record_controlflow_operation_execution_record value) {
	...
	length += kieker_serialize_string(buffer,offset,sessionId);
	length += kieker_serialize_int64(buffer,offset,traceId);
	length += kieker_serialize_int64(buffer,offset,tin);
	length += kieker_serialize_int64(buffer,offset,tout);
	length += kieker_serialize_string(buffer,offset,hostname);
	length += kieker_serialize_int32(buffer,offset,eoi);
	length += kieker_serialize_int32(buffer,offset,ess);
	return length;
}
