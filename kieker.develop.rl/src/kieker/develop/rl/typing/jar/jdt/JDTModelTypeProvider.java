 ...
 * @since 1.2
 */
public class JDTModelTypeProvider implements Resource.Factory, ITypeProvider {
	 ...
	 */
	public JDTModelTypeProvider(final IProject project, final ResourceSet resourceSet) {
	...
	public Iterable<Type> getAllTypes() {
		return IterableExtensions.map(
				IterableExtensions.filter(this.resourceSet.getResource(JDTModelTypeProvider.createResourceURI(), true).getContents(),
						Type.class),
				p -> p);
	}
	...
	public Type findTypeByName(final String name) {
		...
		final JDTModelResource resource = (JDTModelResource) this.resourceSet.getResource(JDTModelTypeProvider.createResourceURI(), true);
	 ...
	 */
	@Override
	public JDTModelResource createResource(final URI uri) {
		return new JDTModelResource(uri, this.project);
	}
