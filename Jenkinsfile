...
node {
	...
	stage ('Prepare') {
		...
		sh 'cd ' + env.WORKSPACE + '/instrumentation-languages; mvn -s settings.xml -B clean'
	}
...
//			sh 'docker run --rm -u `id -u` -v ' + env.WORKSPACE + ':' + LOCAL_PATH + ' ' + DOCKER_IMAGE_NAME + ' /bin/bash -c "cd ' + LOCAL_PATH + '; mvn -X -s settings.xml -B package -Dkeystore=${kdt_key_file} -Dupdatesite=repo@' + REPO_HOST + '/var/www/html"'
			sh 'cd ' + env.WORKSPACE + '/instrumentation-languages; mvn -X -s settings.xml -B package -Dkeystore=${kdt_key_file} -Dupdatesite=repo@' + REPO_HOST + '/var/www/html"
		}
	}
}
