...
#include <stdlib.h>
#include <kieker.h>
#include "c/kieker/common/record/flow/trace/kieker_common_record_flow_trace_trace_metadata.h"
...
int kieker_common_record_flow_trace_trace_metadata_serialize(char *buffer, const int id, const int offset, const kieker_common_record_flow_trace_trace_metadata value) {
	...
	length += kieker_serialize_int64(buffer,offset,threadId);
	length += kieker_serialize_string(buffer,offset,sessionId);
	length += kieker_serialize_string(buffer,offset,hostname);
	length += kieker_serialize_int64(buffer,offset,parentTraceId);
	length += kieker_serialize_int32(buffer,offset,parentOrderId);
	return length;
}
